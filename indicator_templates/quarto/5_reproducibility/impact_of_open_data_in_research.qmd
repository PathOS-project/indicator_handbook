...
This an automated tool used to identify datasets mentioned in the text of publications and extract metadata associated with them, such as name, version, license, etc. This tool can also classify whether the dataset has been introduced by the authors of the publication.

To measure the FWCI for publications that have (re)used open datasets, the tool can be used to identify relevant publications that have introduced open datasets.
...
To use SciNoBo to measure this metric, we would need to input the publications identified from the data sources. This would result in the classification of the publications into specific fields of science and their corresponding FWCI score.

### FWCI for publications that have (re)used open datasets

This metric calculates the Field-Weighted Citation Impact (FWCI) for publications that have (re)used open datasets. It is a measure of the citation impact of research publications that have utilized open datasets, adjusted for differences in citation practices across scientific fields. The FWCI for publications that have (re)used open datasets can indicate the potential impact of data sharing and reuse practices on the visibility and influence of research findings. A higher FWCI score for such publications indicates a higher level of scientific collaboration and sharing of data within a scientific community or field.
...
To measure this metric, we can start by identifying publications that have (re)used open datasets. This can be done by searching for publications in OpenAIRE, or by using automatic tools that extract reused datasets. Once we have identified the relevant publications, we need to determine their discipline in order to calculate their FWCI score. Indicatively, we can use toolkits like SciNoBo or Elsevier’s SciVal, which provide discipline-specific citation impact metadata.
...
This is an automated tool used to identify open datasets mentioned in the text of publications and extract metadata associated with them, such as name, version, license, etc. This tool can also classify whether the dataset has been reused by the authors of the publication.

To measure the FWCI for publications that have (re)used open datasets, the tool can be used to identify relevant publications that have (re)used open datasets.
...
This is an automated tool used to identify datasets mentioned in the text of DMPs and extract metadata associated with them, such as name, version, license, etc. This tool can also classify whether the dataset has been reused by the authors of the publication.

To use the tool to measure the number of datasets reused inside DMPs, we can input a collection of DMPs into the tool and extract the metadata associated with each dataset mentioned in the text. We can then analyze the output to identify which datasets have been reused by the authors of the publication, as classified by the machine learning algorithms of the tool.
