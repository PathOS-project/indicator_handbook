...
The R package “statcheck” (Nuijten et al., 2023) implements the proposed metric. The package can process text strings, PDF and HTML documents, and whole folders containing PDF or HTML documents. In addition, a [web-app based on statcheck](https://michelenuijten.shinyapps.io/statcheck-web/) is available. As discussed above, a key precondition and thus also a limitation for approach used by statcheck is the availability of reported statistics that adhere to the APA format. If statistics are provided completely, statcheck can analyse results from correlations (r) and t, F, χ^2^, Z tests and Q tests^[\[1\]](#footnote-16196){#footnote-ref-16196}^.
